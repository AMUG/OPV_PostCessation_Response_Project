...
elseif( ~isempty(regexp( remain, 'Time_Since_Cessation', 'ONCE')))
    Agemin = ValueTable{j}{TestPoints(i,j)};
    for ii = 1:length(Exp.campaign.Events)
        if strcmpi(Exp.campaign.Events{ii}.Event_Coordinator_Config.Event_Name, 'Initial Immunity - pre-cessation OPV')
            Exp.campaign.Events{ii}.Event_Coordinator_Config.Target_Age_Min = Agemin;
        end
        if strcmpi(Exp.campaign.Events{ii}.Event_Coordinator_Config.Event_Name, 'Initial Immunity - post-cessation IPV')
            Exp.campaign.Events{ii}.Event_Coordinator_Config.Target_Age_Max = Agemin;
        end
    end
