...
Leakage_Cov_Dur_settings = {[0, 0], [0.2, 90], [0.1, 180], [0.05, 365], [0.025, 730]};

s = RandStream('mt19937ar','Seed',1);
RandStream.setGlobalStream(s);

R0_start = 3;
R0_end = 3;  %length(R0_profiles)
campaign_start = 2;
campaign_end = 2; %length(campaign_files)
migration_start = 1;
migration_end = 1; %length(mig_profiles)
leakage_start = 1;
leakage_end = length(Leakage_Cov_Dur_settings);
for curr_mig = migration_start:migration_end
    for curr_R0prof = R0_start:R0_end       
        for currCampaign = campaign_start:campaign_end  
            for currLeakage = leakage_start:leakage_end
                ...
                myjson.EXPERIMENT.Tags.RI_Leakage_Duration = Leakage_Cov_Dur_settings{currLeakage}(2);
                myjson.EXPERIMENT.Tags.RI_Leakage_Coverage = Leakage_Cov_Dur_settings{currLeakage}(1);
                ...
                myjson = loadJson( 'Configs_DTK\campaign.json');
                event_ind = find(cellfun(@(x) ~isempty(regexpi(x.Event_Coordinator_Config.Event_Name, 'OPV post-response RI leakage')), myjson.Events));
                myjson.Events{event_ind}.Event_Coordinator_Config.Intervention_Config.Duration = Leakage_Cov_Dur_settings{currLeakage}(2);
                myjson.Events{event_ind}.Event_Coordinator_Config.Intervention_Config.Demographic_Coverage = Leakage_Cov_Dur_settings{currLeakage}(1);
                saveJson('Configs_DTK\campaign.json', myjson);
                % Merge config files
