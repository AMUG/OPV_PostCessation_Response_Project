function [experiment, numPoints, numReplicates] = NDPolioSweep_and_Calib_ExperimentBuilder(msl_obj, experiment, config, campaign, varargin)
% July 2012, Intellectual Ventures Lab
% TODO: Only works for config parameters for now, not campaign
try
	narginchk(4,4);
catch 
	warning('NDPolioSweep_and_Calib_ExperimentBuilder:Usage', ['Usage: \n', ...
		'NDPolioSweep_and_Calib_ExperimentBuilder(experiment, config, campaign)\n']);
%	rethrow(e);
end
...
TestPoints = [];
if(nargin==6)
    %error('NDPolioSweep_and_Calib_ExperimentBuilder does not yet take external varargin');
...
if( isempty(TestPoints) )
	error('NDPolioSweep_and_Calib_ExperimentBuilder:find_sweep_dim', 'Failed to identify the sweep dimension');
end
...
if numPoints*numReplicates > 20000
   error('NDPolioSweep_and_Calib_ExperimentBuilder:numberofSims', [num2str(numPoints*numReplicates) ' is too many simulations to submit at once.  Reduce sweep points or number of repetitions, or write a batching script']); 
end
...
%                    end
        ...
        if ~exist(fullfile(msl_obj.root, 'input', 'demographics', Curr_Demog_File_Name), 'file')
            ...
            assert(status==0, 'NDPolioSweep_and_Calib_ExperimentBuilder:compiledemog, Error compiling demographics file');
        end
        ...
        if ~isempty(mig_key_str)
            ...
            if ~exist(fullfile(msl_obj.root, 'input', 'migration', Mig_TextFile_Name), 'file')
                ...
                assert(status==0, 'NDPolioSweep_and_Calib_ExperimentBuilder:generate_migration_header, Error generating migration header from demo file');
                ...
                assert(status==0, 'NDPolioSweep_and_Calib_ExperimentBuilder:compilemig, Error compiling migration file');
...
assert(~isempty(base_destination), 'NDPolioSweep_and_Calib_ExperimentBuilder:sendInputFiles, Could not determine input file directory from ES_config.SIMULATION.inputArgString');
        ...
        for currParam = 1:length(CurrParamFields)
            ...
            elseif isfield(this_struct, 'MIN') && isfield(this_struct, 'MAX') &&...
                ...
                elseif strcmpi(this_struct.SPACING, 'LOG')
                    ...
                    error('NDPolioSweep_and_Calib_ExperimentBuilder:setup_sweep', ['Unclear how to define values to sweep over for parameter ' CurrParamFields{currParam} '.  SPACING must be LINEAR or LOG']);
                end
            else
                error('NDPolioSweep_and_Calib_ExperimentBuilder:setup_sweep', 'Unclear how to define values to sweep over.  Define with VALUES array, or MIN, MAX, NPOINTS, SPACING');
