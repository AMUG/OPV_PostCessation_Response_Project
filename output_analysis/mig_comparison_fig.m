...
y_offset = get_migration_mean('quad');
for ii = 1:length(R0f)
        ind =  find(inrange(simconfigs.R0_final, R0f(ii)-.01, R0f(ii)+.01) & inrange(simconfigs.R0_init, .49, .51) & inrange(simconfigs.R0_lambda, 40, 80) & ...
            strcmpi(simconfigs.migration_type, 'quad') & strcmpi(simconfigs.campaign_type, 'campaign_1IPV.json'), 1, 'first');
...
text(0.35, -4.59, 'Migration rate ~ 1/d');
text(0.35, -4.79, 'Migration rate ~ 1/d^2');
myprint('-dpng', 'figures\Fig_mig_new.png');
myprint('-dtiff', 'figures\Fig_mig_new.tiff');
saveas(gcf, 'figures\Fig_mig_new.fig');
end