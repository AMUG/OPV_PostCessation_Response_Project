...
simconfigs = struct('R0_final',       zeros(length(output_dirs), 1), ...
    ...
    'suiteID', {cell(length(output_dirs), 1)});
...
figouts = struct('contour_line', {cell(length(output_dirs), 1)}, ...
    ...
    'var_surface', {cell(length(output_dirs), 1)});
...
save([output_base '\compiled_separatrices_wvar.mat'], 'simconfigs', 'figouts', '-v7.3');
end
...
function [sim_configs] = get_simouts(this_dir, sim_configs, ind2fill)
    ...
    tmp = load([this_dir '\suiteID.mat']);
    sim_configs.folder{ind2fill} = tmp.suiteId;
end
...
function [figouts] = get_figouts(this_dir, figouts, ind2fill)
    ...
    thisfig = openfig([this_dir '\iteration_2\Variance_N1000.fig'], 'invisible');
    figC = get(thisfig, 'children');
    thisax = [];
    for jj = 1:length(figC);
        if strcmpi(class(figC(jj)), 'matlab.graphics.axis.Axes')
    ...
    if ~isempty(thisax)
        ...
        for jj = 1:length(axC)
            ...
            if strcmpi(class(axC(jj)), 'matlab.graphics.chart.primitive.Surface');
                figouts.var_surface{ind2fill}.YData = axC(jj).XData;
                figouts.var_surface{ind2fill}.XData = axC(jj).YData;
                figouts.var_surface{ind2fill}.ZData = axC(jj).ZData;
            end
        end
    end
